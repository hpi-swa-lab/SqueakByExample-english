...
The game board is shown in \figref{gameBoard}; it consists of a rectangular array of light yellow \emph{cells}.
...
How does \squeak{} know that this is the right category?
Well, in general \squeak{} can't know, but in this case, there is also an \ct{initialize} method in a superclass, and \squeak assumes that our \ct{initialize} method should go in the same category as the one that it overrides.
...
Both of them can go in the \prot{initialization} category.
...
\dothis{Place this method in a new category called \prot{game logic}.
	To create the category, press the yellow mouse button in the method category list, select \menu{new category\ldots}, enter ``game logic'', and submit the dialog.}
...
(These are sometimes referred to as \subind{package}{dirty} packages.)
...
What is a package?
For now, you can think of a package as a group of class and method categories that share the same prefix.
...
\dothis{Click on the \button{Save} button to save the first version of your Quinto game on SqueakSource.}
