...
%%{SBESqueakPicture writeTo: './figures/AddCategory.png' frame: [:rect | rect topLeft extent: rect width / 2 @ rect height * 2/3] using: [:helper |
%%	  | window |
%%    Browser open.
%%    window := helper world firstSubmorph.
%%    window bounds: (0 @ 120 extent: 550 @ 375).
%%    helper click: window at: window topLeft + (30 @ 30) buttons: #(right).
%%  	helper hand releaseMouseFocus.
%%    helper mouseEnter: (helper menuItemMorphLabeled: 'add item.*' asRegex in: helper world).
%%    helper closeBalloons.
%%]}
%%{SBESqueakPicture writeTo: './figures/ClassTemplate.png' frame: [:rectangle | rectangle origin extent: rectangle extent * (0.4 @ 0.75)] using: [:helper |
...
%%	helper closeBalloons.
%%	menu := helper world firstSubmorph.
%%	menu bottomRight: window bottomRight - (50 @ 30).
%%	helper hand releaseMouseFocus.
%%	helper mouseEnter: (helper menuItemMorphLabeled: 'accept.*' asRegex in: helper world).
%%]}
\begin{figure}[h!t]
\ifluluelse
...
%%  SBECell new inspect.
%%  window := helper world firstSubmorph.
%%  pane := window findDeepSubmorphThat: #isPluggableListMorph ifAbsent: [self error: 'list not found'].
%%	pane selectionIndex: 3.
%%  window lookFocused.
%%]}
\begin{figure}[htbp]
   \centering
...
%%{SBESqueakPicture writeTo: './figures/SBECellResize.png' frame: [:area | area insetOriginBy: -50 cornerBy: 0] using: [:helper |
%%  | cell window list bottomPane |
%%  cell := SBECell new.
%%  cell inspect.
%%  window := helper world firstSubmorph.
%%  list := window findDeepSubmorphThat: #isPluggableListMorph ifAbsent: [self error: 'list not found'].
%%  list selectionIndex: 3.
...
%%  helper
%%    runModal: [UnknownSelector name: #cellsPerSide]
%%    then: []
%%]}
...
%%  	 ifFail: []]
%%   then: [helper type: (RunArray new: 2 withAll: Character arrowDown) into: helper dialogWindow]
%%]}
\begin{figure}[htb]
...
%%{SBESqueakPicture writeTo: './figures/Categorize.png' frame: [:rect | rect topCenter corner: rect right @ (rect top + (rect height * 2/3))] using: [:helper | | browser class window |
...
%%	| window messageList categoryList startPoint stopPoint |
%%	SBEGame browse.
%%	window := helper world firstSubmorph.
...
%%	startPoint := (helper boundsOfItemMatching: #toggleNeighboursOfCellAt:at: in: messageList) center.
%%	stopPoint := (helper boundsOfItemMatching: 'game logic' in: categoryList) pointAtFraction: 0.2 @ 0.6.
%%	helper mouseDrag: messageList from: startPoint to: stopPoint buttons: #(left).
%%	helper spawnHand.
%%]}
\begin{figure}[htbp]
   \centering
...
%%				helper hand dropMorphs.
%%				game abandon.
%%				stackList := helper listMorphIncludingMatch: '.*toggleNeighboursOfCellAt\:at\:.*' asRegex in: helper world firstSubmorph.
%%				helper keyStroke: stackList listSource key: Character arrowDown.
%%				helper world firstSubmorph extent: 600 @ 400]]
%%]}
\begin{figure}[ht]
\ifluluelse
...
%%	helper type: 'SBE-Quinto' into: packageList listSource.
