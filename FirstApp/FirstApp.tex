...
%%{SBESqueakPicture writeTo: './figures/AddCategory.png' frame: [:bounds | bounds origin extent: bounds extent * (0.4 @ 0.75)] using: [:helper |
%%	| window |
%%	Browser open.
...
%%]}
%%{SBESqueakPicture writeTo: './figures/ClassTemplate.png' frame: [:bounds | bounds origin extent: bounds extent * (0.4 @ 0.75)] using: [:helper |
...
%%	codePane := helper textMorphIn: window withTextMatching: 'SimpleSwitchMorph.*' asRegex.
...
%%  SBECell new inspect.
%%  window := helper world firstSubmorph.
%%	window extent: 315 @ 165.
%%  pane := helper listMorphIncludingMatch: #bounds in: window.
%%	pane listSource changeModelSelection: 3.
%%  window lookFocused.
%%]}
\begin{figure}[htbp]
   \centering
...
%%{SBESqueakPicture writeTo: './figures/Categorize.png' frame: [:rect | rect topRight rect: (rect pointAtFraction: 1 @ 3 / 4) rounded] using: [:helper |
...
%%	| window packageList |
%%	helper mcAssureCachedPackages: #('SBE-Morphic' 'SBE-QuickTour' 'SBE-Quinto').
%%	helper hideProgressMorphDuring: [MCCacheRepository default morphicOpen].
%%	window := helper world firstSubmorph.
%%	packageList := helper listMorphIncludingMatch: 'SBE-Quinto' asRegex in: window.
