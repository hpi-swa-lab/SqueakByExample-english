...
The beauty of \st is that you normally don't need to care about the differences between these three kinds of objects.
...
Method \cmind{Point}{dist:} (\mthref{dist:}) computes the distance between the receiver and another point.
...
All methods are \subind{method}{public}.\footnote{Well, almost all.
	In \squeak, methods whose selectors start with the string \ct{pvt} are private:
	A \ct{pvt} message can be sent \emph{only} to \self.
	However, \ct{pvt} methods are not used very much.}
...
are sent to an instance of \ct{Color}, like the blue color.
...
This means that inheritance, overriding, and super-sends work for class methods in \st, whereas they don't work for static methods in Java.
...
The consequence of this is that the class \ct{WebServer} now has another instance variable, in addition to the variables that it inherits, such as \ct{superclass} and \ct{methodDict}.
...
Since the \ct{uniqueInstance} method is defined in \ct{WebServer class}, you might think that they were the same.
And indeed, until someone creates a subclass of \ct{WebServer}, they are the same.
But suppose that \ct{ReliableWebServer} is a subclass of \ct{WebServer} and inherits the \ct{uniqueInstance} method.
We would clearly expect \ct{ReliableWebServer uniqueInstance} to answer a \lct{ReliableWebServer}:. Using \self ensures that this will happen since it will be bound to the respective class.
...
\dothis{A new class is normally created by sending the message \ct{subclass: instanceVariableNames: ...} to an existing class.
...
Although \squeak does not provide multiple inheritance, it has incorporated a mechanism called \emphind{trait}{}s for sharing behavior across unrelated classes.
...
By contrast, the meaning of \self is dynamic: it always represents the receiver of the currently executing message.
...
\item 
\glbind{ActiveHand} is the current instance of \clsind{HandMorph}, the graphical representation of the cursor.
\ct{ActiveHand}'s submorphs hold anything being dragged by the mouse.
...
Inspecting \ct{Undeclared} can sometimes help explain strange behavior!

\item
\glbind{SystemOrganization} is an instance of \clsind{SystemOrganizer}:
It records the organization of classes into packages.
More precisely, it categorizes the \emph{names} of classes, so
\begin{code}{@TEST}
...
Current practice is to strictly limit the use of global variables; it is usually better to use class instance variables or class variables, and to provide class methods to access them.
...
The usual way to define a global is just to \menu{do it} on an assignment to a capitalized but undeclared identifier.
...
For example, the class \clsind{Text} indicates that it is using the pool dictionary \ct{TextConstants}, which contains all the text constants such as \glbind{CR} and \glbind{LF}. 
