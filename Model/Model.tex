...
You will see the effect of executing this method directly within the world. (Select \menu{World \go \ind{restore display}~(r)} to undo the effects.)
...
\begin{classdef}[sebwebserver]{The class side of the singleton class}
SBEWebServer class 	
	instanceVariableNames: 'uniqueInstance'
\end{classdef}

The consequence of this is that the class \ct{SBEWebServer} now has another instance variable,
...
What is the difference?   Since the \ct{uniqueInstance} method is defined in \ct{SBEWebServer class}, you might think that they were the same.    And indeed, until someone creates a subclass of \ct{SBEWebServer}, they are the same.  But suppose that \ct{SEBReliableWebServer} is a subclass of \ct{SBEWebServer}, and inherits the \ct{uniqueInstance} method.  We would clearly expect \ct{SEBReliableWebServer uniqueInstance} to answer a \lct{SEBReliableWebServer}. Using \self ensures that this will happen, since it will be bound to the respective class.
Note also that \ct{SBEWebServer} and \ct{SEBReliableWebServer} will each have their own class instance variable called \ct{uniqueInstance}. These two variables will of course have different values.
...
Although \squeak does not provide multiple inheritance, it incorporates a mechanism called \emphind{trait}{}s for sharing behaviour across unrelated classes.
...
Also, global variables introduce \emph{global state} as mentioned earlier which impedes the modularity of \st objects and hinders us to use a \st object directly without taking care of its environment.
