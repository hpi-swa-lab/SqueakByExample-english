...
Then, we click on the \button{class} button to switch to the \subind{system browser}{class side} and add the instance variable \ct{uniqueInstance} there.
...
This would have caused the lookup to start from \ct{BorderedMorph}, resulting in an infinite loop!
...
When this method is not found, the \ind{virtual machine} will notify the object of the failure by causing it to send \ct{self doesNotUnderstand: #foo}.
...
\newline % To push the class definition on the next page
