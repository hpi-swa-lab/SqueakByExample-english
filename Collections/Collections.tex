...
#() fold: [:sum :each | sum + each] --> !\emph{Error: Cannot fold over an empty collection.}!
...
\mthind{Symbol}{ascending} and \mthind{BlockClosure}{descending} are defined on \clsind{Symbol} as well, so we could also write the previous example in the following way (however, the former version is more efficient, easier to debug, and often would be considered better style):
