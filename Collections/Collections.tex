...
%			<< '\begin{itemize}';
%			cr.
%		(hierarchy sorted: [:assoc | assoc key name] ascending)
...
\hline
\protind{copying}
& \lct{,}, \lct{copyWith:}, \lct{copyWithout:} \\
\hline
\end{tabular}
...
\st provides a number of different kinds of collections, that all are represented as subclasses of \clsindmain{Collection}.
In \squeak, the abstract class \ct{Collection} has 101 subclasses, but many of these (like \mbox{\clsind{Bitmap},} or \clsind{CompiledMethod}) are special-purpose classes crafted for use in other parts of the system or in applications, and hence not categorized as ``Collections'' by the system organization.
For the purposes of this chapter, we use the term ``Collection Hierarchy'' to mean \ct{Collection} and its 65 subclasses that are \emph{also} in the system categories labeled \scat{Collections-*}.
In this chapter, we focus mainly on the subset of collection classes shown in \figref{CollClassesList}\footnote{The group of classes that appears in the ``\ind{Blue Book}''~\cite{Gold83a} contains 17 subclasses of \ct{Collection}, and had already been redesigned several times before the \st-80 system was released.
...
\emph{All} collections understand the \prot{enumeration} messages \mthind{Collection}{do:}, \mthind{Collection}{select:} (which is like \ct{filter} in \ind{Lisp}, JavaScript, and others), \mthind{Collection}{reject:} (which is the opposite of \ct{select:}), \mthind{Collection}{collect:} (which is like \ct{map} in Lisp \etc), \mthind{Collection}{detect:ifNone:}, \mthind{Collection}{inject:into:} (which performs a left fold), and many more.
...
\begin{itemize}
  ...
  \item {\bf Growable:}
    (Sub-)instances of \ct{Interval} and \clsind{ArrayedCollection} are always of a fixed size.
    Other kinds of collections (sorted collections, ordered collections, and linked lists) can grow after creation.

	The class \clsind{OrderedCollection} is more flexible than \ct{Array}; the size of an \ct{OrderedCollection} grows on demand, and it has methods for \mthind{OrderedCollection}{addFirst:} and \mthind{OrderedCollection}{addLast:} as well as \mthind{OrderedCollection}{at:} and \mthind{OrderedCollection}{at:put:}.
  ...
  \item {\bf Heterogeneous:}
	...
	An \clsind{Array} will hold any mix of objects, but this is not true for \clsindplural{RawBitsArray}: a \lct{ByteArray} only holds single bytes (\ie \clsindplural{SmallInteger} between \ct{0} and {255}), a \clsind{FloatArray} only holds \clsindplural{Float}, a \clsind{ColorArray} only holds \clsindplural{Color} \etc*.
...
\begin{enumerate}
  \item \clsindplural{ArrayedCollection} store their elements in the (indexable) instance variables of the collection object itself; as a consequence, arrays must be of a fixed size, but can be created with a single memory allocation message.
  \clsindex{Array}
  \item \clsindplural{OrderedCollection} and \clsindplural{SortedCollection} store their elements in an array that is referenced by one of the instance variables of the collection.
  Consequently, the internal array can be replaced with a larger one if the collection grows beyond its storage capacity.
  \item The various kinds of \clsind{Set} and \clsind{Dictionary} also reference a subsidiary array for storage but use the array as a hash table.
  \clsindplural{Bag} use a subsidiary \ct{Dictionary}, with the elements of the bag as keys and the number of occurrences as values.
  \item \clsindplural{LinkedList} use a standard singly-linked representation.
  \item \clsindplural{Interval} are represented by three integers that record the two endpoints and the step size.
\end{enumerate}
...
Set withAll: #(7 3 1 3)                         --> !\SqVersionSwitch{Trunk}{a Set(1 3 7)}{a Set(7 1 3)}!
Bag withAll: #(7 3 1 3)                        --> !\SqVersionSwitch{Trunk}{a Bag(1->1 3->2 7->1)}{a Bag(7 1 3 3)}!
\end{code}
...
Beware that \ct{at:put:} returns the argument and not the receiver, so you often need to build a cascade \mthind{Object}{yourself}!
...
Array literals can contain numbers, \ct{nil}, \ct{true}, \ct{false}, characters, strings, symbols, and nested array literals.
\subindex{Array}{literal}

Analogously, \emph{byte arrays} that are created using \ct{#[ ]} are literal as well.
\subindex{ByteArray}{literal}
\seeindex{\#@{\textsf{\#[ ]}}}{ByteArray, literal}
...
\subindex{Array}{brace construct}
\seeindex{\{@{\textsf{\{ \}}}}{Array, brace construct}
...
Unless you copy the array, it will be a \emphind{read-only object} and signal a \clsind{ModificationForbidden} error every time you try to alter it.
...
Similarly to \cmind{Collection}{at:put:}, all \ct{add} methods return the argument and not the receiver.

\paragraph{Removing Elements.}
The method \mthind{OrderedCollection}{remove:} \ct{anObject} removes the first occurrence of an object from the collection and returns it.
...
If your dictionary only uses \clsindplural{Symbol} as keys, you can also instead use an \ct{IdentityDictionary} since every symbol is guaranteed to be globally unique.
...
\noindent
The result of the example is \ct{2}, as the color white was included twice in the collection. Once as the result of \ct{Color white}, and once as the result of the combination of red, blue, and green.
However, if you are working with a sequenceable collection and want to preserve its type or order, you can use \mthind{SequenceableCollection}{withoutDuplicates} instead.
...
The receiver may be first converted to a \ct{Set}, so these operations work for all kinds of collections!
...
\end{code}
\noindent
The correct answer is to either use \cmind{String class}{newFrom:}, \cmind{String class}{withAll:}, or \cmind{Object}{as:}:
\begin{code}{@TEST}
...
For example, the class \ct{Color} is not a Magnitude and it does not implement the method \ct{<=}, but we can specify that the colors should be sorted according to their luminance (a measure of brightness).
...
We will explain sort functions in more detail in \secref{sorting}.
...
\seeindex{Collection!concatenation}{Collection, \ct{,}}
\seeindex{,}{Collection, \ct{,}}
\seeindex{comma}{Collection, concatenation}
\cmindex{Collection}{,}
\begin{code}{@TEST |s|}
s := 'no' , ' ' , 'worries'.
s -->  'no worries'
...
Since a string is a mutable collection (unless it is a method literal) we can also change it using the method \mthind{String}{at:put:}.
...
(1 to: 3) , '45' --> #(1 2 3 $4 $5)
\end{code}
\seeindex{String!concatenation}{String, \ct{,}}
\cmindex{String}{,}
...
More advanced pattern matching facilities based on \ind{regular expressions} are provided by the \ind{Regex} package.}
...
If you look up the implementation of \ct{collect:}, \ct{do:}, \autc in any collection class, you will find that they evaluate the argument block by sending the message \mthind{BlockClosure}{value:} to it.
That means that you actually can pass any other object that understands the message \ct{value:} to \ct{collect:}, \ct{do:}, \etc*.
In particular, this includes the classes \clsind{Symbol} and \clsind{MessageSend}.
Thus, you could also write the above expression as follows:
\begin{code}{@TEST}
#(2 -3 4 -35 4 -11) collect: #abs --> #(2 3 4 35 4 11)
\end{code}
\cmindex{Symbol}{value:}
\cmindex{MessageSend}{value:}
\noindent
While using a \subind{message}{selector} symbol instead of a block is a popular shortcut, it is not always considered good style.
...
The method \mthind{Collection}{detect:} returns the first element of the receiver that matches the block argument.
...
In \sq this is done by \cmind{Collection}{inject:into:} or \cmind{Collection}{fold:} (also available as \cmind{Collection}{reduce:}).

The first argument of \ct{inject:into:} is an initial value, and the second argument is a two-argument block which is applied to the result this far, and each element in turn.
...
\end{code}
\noindent
Because \ct{Symbol} also understands \cmind{Symbol}{value:value:}, we can also write this:
\begin{code}{@TEST}
...
\ct{fold:} is a simpler version of \ct{inject:into:} that uses the first element of the collection as the initial value.
However, it signals an error if the collection is empty, whereas \ct{inject:into:} lets you specify a default value in that case.
...
\paragraph{\mthind{Collection}{anySatisfy:}.}
...
Analogously, there are also \strong{\mthind{Collection}{allSatisfy:}} and \strong{\mthind{Collection}{noneSatisfy:}}.
...
\paragraph{\mthind{Collection}{gather:}.}
...
A difficult error to spot is when you redefine \mthind{Object}{=} but not \mthind{Object}{hash}.
The symptoms are that you will lose elements that you put in sets or other strange behavior.
One solution proposed by Kent Beck is to use \mthind{Integer}{bitXor:} to redefine \ct{hash}.
...
(If you need to do it anyway, you can send the message \mthind{HashedCollection}{rehash} to the collection to force it to recompute the hash values of all its elements.)
...
\mthind{Symbol}{ascending} and \mthind{Symbol}{descending} are defined on \clsind{Symbol} as well, so we could also write the previous example in the following way:
...
For further information on sort functions, you can browse the system category \scatind{Collections-SortFunctions} and its extension methods on \ct{BlockClosure} and \ct{Symbol}.
...
\begin{itemize}
  ...
  \item To sort a collection, send it the message \ct{sorted} or \ct{sorted:}, to sort it in-place, send \ct{sort} or \ct{sort:}, and to create a collection that always keeps its element sorted send \ct{asSortedCollection} or \ct{asSortedCollection:}.
