...
\emph{All} collections understand the \prot{enumeration} messages \mthind{Collection}{do:}, \mthind{Collection}{select:}, \mthind{Collection}{reject:} (which is the opposite of \ct{select:}) (which is like \ct{filter} in \ind{Lisp}, JavaScript, and others), \mthind{Collection}{collect:} (which is like \ct{map} in Lisp \etc), \mthind{Collection}{detect:ifNone:}, \mthind{Collection}{inject:into:} (which performs a left fold), and many more.
...
Take care that \ct{at:put:} returns the argument and not the receiver, so you often need to build a cascade \mthind{Object}{yourself}!
...
Dynamically created arrays do not have this problem.
...
#(2 -3 4 -35 4 -11) collect: (Message selector: #abs) --> #(2 3 4 35 4 11)
\end{code}
