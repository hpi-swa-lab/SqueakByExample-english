...
Sort methods use the message \mthind{Magnitude}{<=} to establish sort order, so by default they can sort magnitudes such as numbers, characters, times \etc (\seeindex{Magnitude}), but also other objects such as strings or points.
However, what can you do if you want to sort objects that do not implement \ct{<=}, or if you would like to customize sort order?
...
These messages take a two-argument sort block or a \emphind{sort function}\index{SortFunction} that, speaking formally, defines a partial order on the collection to be sorted.%
...
\begin{code}{@TEST}
#('Foo' 'Bar' 'baz') sorted:
    [:a :b | a caseInsensitiveLessOrEqual: b] --> #('Bar' 'baz' 'Foo')
\end{code}
...
\mthind{Symbol}{ascending} and \mthind{BlockClosure}{descending} are defined on \clsind{Symbol} as well, so we could also write the previous example in the following way (but the former version is more efficient):

\begin{code}{@TEST}
#('Foo' 'Bar' 'baz') sorted: #asUppercase descending --> #('Foo' 'Bar' 'baz')
\end{code}
...
\begin{code}{@TEST}
{1. 100. nil. 1000. 10} sorted:
	[:ea | ea ifNotNil: [ea asWords first]] ascending undefinedLast
		, SortFunction default reversed. --> #(1000 100 1 10 nil)
\end{code}
