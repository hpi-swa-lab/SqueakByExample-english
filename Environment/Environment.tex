...
The \menu{senders} button lists \emph{all} methods that send the chosen message: not all of these sends will result in the execution of the method you are looking at.
Indeed, much of the power of object-oriented programming comes from the fact that message sends are \emph{polymorphic}, that is, they can work equally well on objects of any class.
Sometimes it is easy to figure out which method will be executed as the result of a message send, and sometimes it is impossible; the senders and implementors tools don't try.
...
\vspace{\baselineskip} % To push one line of the next paragraph on the next page
...
In the category pane, it files out the whole category, in the class pane it files out the whole class, in the protocol pane it files out the whole protocol, and in the method pane it files out just the displayed method.
...
\index{keyboard shortcuts!cancel}
...
\index{keyboard shortcuts!search bar}
