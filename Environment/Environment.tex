...
%%	helper select: list at: 'Canvas draw:*' andScrollIntoViewBy: 3.
%%]}
\begin{figure}[htbp]
	\begin{center}
   \ifluluelse
...
%%	helper scaleWindow: helper foregroundWindow extent: 500 @ 250.
%%	pane := helper listMorphIncludingMatch: '*Morph*' in: helper foregroundWindow.
%%	helper select: pane at: '*Morph*'
%%]}
\begin{figure}[tbp]
	\begin{center}
   \ifluluelse
...
.
	...
	recorder writeTo: 'debuggerNotFound.png' building: [:helper |
		...
		codePane := helper textMorphIn: debugger withTextMatching: 'errorNotFound:*'.
...
\begin{figure}[btp]
	\begin{center}
	\ifluluelse
		{\includegraphics[width=\textwidth]{debuggerNotFound}}
		{\includegraphics[scale=0.7]{debuggerNotFound}}
	\end{center}
	\caption{The debugger.}
	\label{fig:debuggerNotFound}
\end{figure}
...
The debugger is shown in \figref{debuggerNotFound}; it looks intimidating at first, but it is quite easy to use.
...
In \figref{debuggerNotFound} we have selected the \ct{detect:ifFound:ifNone:} method in the top pane.
...
^ self copyFrom: dotPosition to: self size'
.
	allTests := StringSuffixTest selectors select: #isTestSelector.
	thisContext wrap: [:block | helper restoreMethods: (allTests collect: [:test | StringSuffixTest >> test]) after: block].
	(allTests copyWithout: #testSuffixFound) do: [:test | StringSuffixTest removeSelector: test].
	testRunner := TestRunner newForSuite: StringSuffixTest suite.
...
buildChangeSet := [:helper :outerBlock |
	...
	methods := {
		String >> #suffix.
		StringSuffixTest >> #testSuffixFound }.
