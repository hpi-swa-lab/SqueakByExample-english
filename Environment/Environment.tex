...
.
	...
	recorder writeTo: 'debuggerDetect.png' building: [:helper |
		...
		helper scaleDefaultDebugger: helper foregroundWindow.
		targetContext := '*detect:ifFound:ifNone:'.
		stackList := helper listMorphIncludingMatch: targetContext in: helper foregroundWindow.
		helper select: stackList at: targetContext.
		helper mouseEnter: (helper textMorphIn: helper foregroundWindow withTextMatching: 'detect:*').
	].
	recorder writeTo: 'RestartDetect.png' building: [:helper |
		helper click: (helper buttonLabeled: 'Restart*' in: helper foregroundWindow).
	].
	recorder writeTo: 'steppingIntoValue.png' building: [:helper |
		| stepThrough contextVarList debugger |
		stepThrough := helper buttonLabeled: 'Through*' in: helper foregroundWindow.
		contextVarList := helper listMorphIncludingMatch: #thisContext in: helper foregroundWindow.
		helper click: stepThrough.
		helper select: contextVarList at: #each asString.
		debugger := helper foregroundWindow model.
		[((debugger instVarNamed: #contextVariablesInspector) object tempNamed: #each) > 7]
			whileTrue: [helper click: stepThrough].
	].
	recorder writeTo: 'dotIsAString.png' building: [:helper |
		helper click: (helper buttonLabeled: 'Into*' in: helper foregroundWindow).
	].
]
\end{ExecuteSmalltalkScript}
\begin{figure}[btp]
...
\begin{figure}[btp]
	\begin{center}
	\ifluluelse
		{\includegraphics[width=\textwidth]{debuggerDetect}}
