...
Also available from the yellow button menu on the class pane is \menu{class refs (N)}, which displays a list of all of the methods that directly reference the current class.
...
\begin{itemize}
\item Category \ct{'SqueakLink-Connections'} contains \ct{OracleConnection MySQLConnection PostgresConnection}
\item Category \ct{'SqueakLink-Model'} contains \ct{DBTable DBRow DBQuery}
\end{itemize}
...
Click on the \emph{debug} hayndle \debugHandle{} and select \menu{explore morph}
...
There are three tools in \sq to help you find messages; the first two, MethodFinder and MessageNames, can be opened from \menu{World docking bar\go{}Tools}, the third one is the global search in the right corner of the world docking bar.
...
Beyond the common tools introduced in this chapter, \sq also provides a set of other interesting and rich tools.
We introduce some of them which you might want to use after becoming more proficient in using \sq:

\paragraph{Dependency browser}
Whenever you use a class in a method, the method from then on depends on the presence of this class.
If the class is not in the environment, your method will result in an exception, as \sq does not who should receive the message.
...
Using the \ind{dependency browser} you can see the dependencies of your package down to the individual methods.
You can open the dependency browser through \menu{world main docking bar \go Apps \go Dependency Browser}.

\paragraph{Git tools}
\ind{Git} is a common version control system.
For sharing you projects via Git on platforms such as GitHub, GitLab, or Bitbucket, you can use the Git tools.
However, the tools are not part of the standard environment and you will have to install them.
...
\paragraph{Lexicon}
The browser only shows the messages implemented by a class.
Instances of that class understand many more messages, for example by messages implemented by super classes of their class.
...
You can either let it measure the times for all executions in the image through \menu{world main docking bar \go Extras \go Start Profiler}.
