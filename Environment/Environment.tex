...
First, the method would be displayed as missing in the original package, so the package would unintentionally become \emph{dirty} (\ie it would be considered containing changes that were not yet saved to any repository).
...
% ct: In fact, e.g., Kernel and Kernel-Exceptions and could be defined as overlapping packages. However, this is commonly not an intended usage of packages so we omit it for sake of simplicity here.
...
\begin{ExecuteSmalltalkScript}
"SBEMonticelloSeries"
SBEScreenshotRecorder writeInto: './figures' record
 [:picture :helper |
