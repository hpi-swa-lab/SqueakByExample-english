...
In the class browser, you can look through the methods to learn what they should do, but you can not execute them and see what they actually do.
In the inspector or explorer, you can see the state of an object and execute methods, but you can not directly see the code corresponding to the methods.
...
In the new browser (\figref{fig:instanceBrowser}), you can see a list of methods on the top right, which contains all methods the object can execute, gathered from its class and all its superclasses.
...
However, this window is special as it still knows about the \ct{PluggableListMorph} for which we opened it.
Thus, we can evaluate code in the code pane.
\dothis{In the browser, open the method \ct{fullListSize} by selecting it in the list at the top right. Then navigate to the last code line and \menu{print it} (\short{p}). 
You should see the result of evaluating the expression right within the code editor.}
