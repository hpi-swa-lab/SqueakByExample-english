...
String newFrom: collection asArray.
\end{code}
...
You can add \ct{TextAttribute}s to section of text by using the messages in the \prot{emphasis} protocol of \cls{Text}, for example:
\begin{code}{}
...
\end{faq}
\answer
There is a tool called \ct{ProcessBrowser} which you can either open from \menu{World menu \go open} or \menu{World docking bar \go Tools}. 
The \ind{ProcessBrowser} lists all processes and the yellow button menu allows you to control the process, \eg terminate it.
