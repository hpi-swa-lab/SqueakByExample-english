...
\begin{itemize}
    \item \mthind{Exception}{return:} returns a value from the handler block.
    ...
    [self downloadData]
        on: ConnectionRefused do: [:ex |
            self cache at: ex host ifPresent: [:cached | ex return: cached].
            ex pass].
    \end{code}

    \item \mthind{Exception}{resume} continues the execution of the method that raised the exception\footnote{Most other languages not support resumable exceptions. However, there are some similar concepts such as \ct{On Error Resume Next} in Visual Basic, \ct{begin...rescue...retry} in Ruby, or \ct{restart-case} in Common Lisp.}.
    ...
    However, specifically for handling \clsindplural{ProvideAnswerNotification}, there are the more convenient methods \cmind{BlockClosure}{valueSupplyingAnswers:} and \cmind{BlockClosure}{valueSuppressingMessages:}.
