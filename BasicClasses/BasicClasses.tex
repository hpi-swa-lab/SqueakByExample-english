...
If you want to pass a message along to help users figure out what happened, you can use \cmind{Exception}{signal:}.
...
To catch an exception you can use the method \cmind{BlockClosure}{on:do:}\footnote{This is similar to \ct{try...catch} in other languages.}, which takes an exception class (or \ct{ExceptionSet}) as the first argument and a block as the second.
...
% - Resuming
% - Passing along state
% As we sometimes want to catch several kinds of exceptions, you can combine \ct{Exception}s into an \ct{ExceptionSet} by sending the message \cmind{Exception}{,}.
