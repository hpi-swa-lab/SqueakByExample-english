...
In the following chapter \charef{collections}, we will discuss the equally important \ct{Collection} subclasses.
...
\cmind{Object}{printOn:} is very likely one of the methods that you will most frequently override.
This method takes as its argument a \clsind{Stream} (for more on streams see \charef{streams}) on which a \clsind{String} representation of the object will be written.
The default implementation simply writes the class name preceded by ``\ct{a}'' or ``\ct{an}''.
\cmind{Object}{printString} returns the \ct{String} that is written.
...
The default implementation of \cmind{Object}{~=} simply negates \ct{Object>>>=}, and should not normally need to be changed.
...
\important{If you override \ct{=}, you should override \mthind{Object}{hash}.}
...
Another way to copy an object is provided by \cmind{Object}{veryDeepCopy}:
...
Otherwise, an \clsind{AssertionFailure} exception will be raised, which is a subclass of \ct{Error}.
...
Do not confuse \cmind{Object}{assert:} with \cmind{TestCase}{assert:}, which occurs in the SUnit testing framework and raises a \clsind{TestFailure} (see \charef{SUnit}).
While the latter also has variants such as \ct{assert:equals:} or \ct{deny:}, the former has no variants.
...
Sending \lct{self deprecated: \emph{anExplanationString}} signals that the current method should no longer be used, if deprecation warnings have been turned on in the ``Debug''category of the \ind{preference browser}.
...
The methods in \protind{testing} are not related to unit tests!
A testing method lets you ask a question about the state of the receiver and returns a \clsind{Boolean}.
...
\caption{The number hierarchy (excerpt).\label{fig:numbers}}
\end{figure}
% TODO: ct: insert ScaledDecimal under number (right to Integer)
...
(\clsind{Complex} numbers are not comparable, and so do not inherit from \clsind{Number} nor from \clsind{Magnitude}.)

The methods \cmind{Magnitude}{<} and \cmind{Magnitude}{=} are abstract.
...
\cmind{Number}{printOn:} is implemented in terms of the abstract method \cmind{Number}{printOn:base:}.
...
\emph{Rounding} methods include \mthind{Number}{rounded}, \mthind{Number}{roundTo:}, \mthind{Number}{roundUpTo:}, and so on.
...
\clsindplural{Fraction} are represented by instance variables for the numerator and denominator, which should be \ct{Integer}s.
...
\clsindmain{ScaledDecimal} represents a decimal number with a fixed number of digits after the decimal point.
\index{scaled decimal}
Other than floats, they do not suffer from rounding errors and can represent numbers of arbitrary size; however, they also take up more space and time to calculate.

You can create a literal scaled decimal by putting the letter \ct{s} at the end of a literal number, followed by an integer that specifies the number of digits after the decimal point.
If you omit the scale after the \ct{s}, the number of visible digits will be used:

\begin{code}{@TEST}
2.5    --> 2.5         "a Float"
2.5s1 --> 2.5s1     "a ScaledDecimal"
2.5s2 --> 2.50s2   "same value, but with a higher precision"
2.5s0 --> 2s0        "same value, but with a lower precision"
2.5s  --> 2.5s1      "same value, but with automatic precision"
2s2 --> 2.00s2      "a ScaledDecimal without a decimal point"
\end{code}

You can also create a \ct{ScaledDecimal} from another number by sending it the message \mthind{Number}{asScaledDecimal:}, indicating the number of digits after the decimal point:

\begin{code}{@TEST}
(1.342 asScaledDecimal: 2) --> 1.34s2
\end{code}
...
Should you need more advanced support for \ind{regular expressions}, you can also use the \ct{Regex} extension methods defined on \ct{String} from the \ct{Regex-Core} package.
For example:
...
There are also a number of useful methods for converting a string to another string, such as \mthind{String}{capitalized} and \mthind{String}{asLowercase}.
...
As you have seen, many classes such as \ct{String} or \ct{Integer} provide you with dozens of useful selectors, and we only introduced a few of them here.
...
When you are trying to do something with some object while writing a script or application, it is often a good idea to keep a \ind{browser} browser open (see \secref{browsing}) on its class and consult it from time to time as you are searching for a specific functionality.

\dothis{Open a browser on the class \ct{String}, focus the message list, and type \ct{numb} to discover messages that you can send your string to convert it into a number.}

Also remember that you can use the method finder (see \secref{methodFinder}) for finding messages based on their output rather than their name, which works especially well for strings, collections, and numbers.
...
\clsind{Exception} might not be the first class you will need when starting to explore \sq.
...
In particular, subclasses of \clsind{Error} represent erroneous situations, and subclasses of \clsind{Notification} indicate that something interesting occurred.

To signal (or raise) an exception, you can use the message \mthind{Exception}{signal}.
If you want to pass a message along to help users figure out what happened, you can use \mthind{Exception}{signal:}.
When an exception is not handled by anyone, it is sent the message \mthind{Exception}{defaultAction} which is implemented differently by different types of exceptions.
The default action for \ct{Error}s is to open a debugger, while the default action for \ct{Notification}s is to do nothing but proceed with the execution of the program:

\begin{code}{NB: CANNOT TEST}
Error signal --> "Will open a debugger"
Error signal: 'We wanted this to happen' --> "Will open a debugger with a message"
Notification signal: 'We wanted this to happen' --> "Nothing happensBANG"
\end{code}

As we do not want to present users with debuggers every time something unexpected happens, we can also \emph{handle} exceptions directly within our code.
...
To catch an exception, you can use the method \cmind{BlockClosure}{on:do:}\footnote{This is similar to \ct{try...catch} in other languages.}, which takes an exception class as the first argument and a block as the second:
...
The code above does not return a collection, as the block was interrupted by the \ct{ZeroDivide} exception before it was able to return the result of the \ct{#collect:} send.
Instead of doing nothing, you can also return another value from the handler block:

\begin{code}{@TEST | result |}
result := (-2 to: 2) collect: [:i |
    [100 / i]
        on: ZeroDivide
        do: [:ex | Float nan]].
result --> {-50 . -100 . NaN . 100 . 50}
\end{code}

There are also ways to handle multiple exceptions at once.
You can combine many exceptions into an \ct{ExceptionSet} using the method \cmind{Exception}{,} and pass that \ct{ExceptionSet} to \ct{on:do:}.
Another option is to use \cmind{BlockClosure}{on:do:on:do:}, \ct{BlockClosure>>>on:do:on:do:on:do:}, \etc for attaching multiple different handlers to a block.

Sometimes you might not be interested in handling an exception.
Instead, you might only want to \emph{ensure} that some code is always executed after your block, regardless of whether an exception occurs, the block returns early, or the block finishes normally.
To achieve this, use the method \cmind{BlockClosure}{ensure:}\footnote{This is similar to \ct{finally} in other languages.} which will always execute the block passed to it.
...
Note that under normal circumstances, you should never place a return statement (\ct{^}) in an ensure block.
If you do so, the execution will not reach further \ct{ensure:} blocks but will continue as if no exception had occurred.
In particular, this can create a zombie process when you abandon a program from a debugger.

So far, we only have \emph{returned} from an exception.
However, exception handlers have a couple of other options for dealing with exceptions by sending messages to them:

\begin{itemize}
    \item \mthind{Exception}{pass} passes the exception to the next handler on the stack\footnote{This is similar to a plain \ct{throw} in other languages.} (or to the \mthind{Exception}{defaultAction}, if there is no further exception handler).
    Unlike sending \mthind{Exception}{signal} again to the exception, \ct{pass} will maintain the existing signaler stack, which is usually desired behavior (as you can debug and resume from the original exception).
    ...
    If the data is in that cache, it will return it.
    If the cache does not contain the data either, it will pass the exception to the next handler, which will open a debugger.

    \item \mthind{Exception}{resume} continues the execution of the method that signaled the exception\footnote{Most other languages do not support resumable exceptions. However, there are some similar concepts such as \ct{On Error Resume Next} in Visual Basic, \ct{begin...rescue...retry} in Ruby, or \ct{restart-case} in Common Lisp.}.
    This is similar to pressing \menu{Proceed} in the debugger for an exception.
    You can also pass a value along with \mthind{Exception}{resume:} which will be returned from the original \mthind{Exception}{signal:} send.
    This is a powerful mechanism that allows your code to communicate with other methods on the stack that it does not know about.
    ...
    \item \mthind{Exception}{retry} restarts the execution of the block that signaled the exception.
    However, you should use this with caution as the exact method that will signal an exception is often unknown, and restarting the wrong method can lead to unexpected behavior and infinite loops.

    \item \mthind{Exception}{return} returns from the handler, and \mthind{Exception}{return:} passes along a value.
    This is the implicit default behavior of every handler block.
    The advantage of using \ct{return} explicitly is that you can leave the handler block immediately, similar to a \ct{^} return statement in a method.
...
As exceptions are instances of classes like any other objects, it is a good practice to define your own exceptions by subclassing \clsind{Error} or \clsind{Notification}.
This allows you to handle them with ease and to add instance variables and accessors, which can be useful for passing additional information to the exception handler.
There are other subclasses of \ct{Exception} as well, but most of the time you will not have to deal with them.
In particular, you should never instantiate, subclass, or handle the abstract base class \ct{Exception} itself.
...
%  \item Send \ct{yourself} to get back the receiver at the end of a cascade.
  ...
  \item Signal errors using \ct{Object>>>error:} or \ct{Exception>>>signal:} and handle them using \ct{BlockClosure>>>on:do:}.

  \item You can return, resume, pass, or retry exceptions.
