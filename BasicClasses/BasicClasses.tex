...
Should you need more advanced support for \ind{regular expressions}, you can also use the \ct{Regex} extension methods defined on \ct{String} from the \ct{Regex-Core} package that was originally developed by Vassili Bykov.
\index{Regex}
For example:
...
To catch an exception you can use the method \ct{BlockClosure>>>on:do:}\cmindex{BlockClosure}{on:do:}\footnote{This is similar to \ct{try...catch} in other languages.}, which takes an exception class (or \ct{ExceptionSet}) as the first argument and a block as the second.
...
To achieve this, use the method \ct{BlockClosure>>>ensure:}\footnote{This is similar to \ct{finally} in other languages.} which will always execute the block passed to it.
