...
To raise an exception, you can use the message \mthind{Exception}{signal}.
...
\begin{itemize}
    ...
    \item \mthind{Exception}{resume} continues the execution of the method that raised the exception\footnote{Most other languages do not support resumable exceptions. However, there are some similar concepts such as \ct{On Error Resume Next} in Visual Basic, \ct{begin...rescue...retry} in Ruby, or \ct{restart-case} in Common Lisp.}.
    ...
    \item \mthind{Exception}{retry} restarts the execution of the block that raised the exception.
    However, you should use this with caution as the exact method that will raise an exception is often unknown, and restarting the wrong method can lead to unexpected behavior and infinite loops.
...
%  \item Send \ct{yourself} to get back the receiver at the end of a cascade.
  ...
  \item Raise errors using \ct{Object>>>error:} or \ct{Exception>>>signal:} and handle them using \ct{BlockClosure>>>on:do:}.
