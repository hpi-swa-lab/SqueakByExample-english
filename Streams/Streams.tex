...
\figref{ax_cde} shows the state of the same stream after having written an \ct{x} using the message \mthind{Stream}{nextPut:} \ct{anElement}.
...
Imagine you want to create a \ct{History} class which will manage backward and forward buttons in a web browser.
...
To do this, the simplest solution is to write \ct{nil} just after to indicate the history end:
...
That's why \ct{FileStream} offers a message called \mthind{FileStream class}{forceNewFileNamed:do:} to automatically close a new stream after evaluating a block that sets its contents.
