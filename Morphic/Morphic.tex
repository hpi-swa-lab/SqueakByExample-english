...
\hfill
\begin{ExecuteSmalltalkScript}
SBEScreenshotRecorder debugging writeTo: 'figures/popup.png' building: [:helper |
	helper runModal: [
		Project uiManager
...
To allow developers to write dialogs that work for any UI system, \sq provides the \clsind{UIManager} facade that defines a consistent interface to invoke user dialogs for any UI system.
Every UI system then is represented by a subinstance of the class \clsind{Project} which instantiates a customized subclass of the \clsind{UIManager}.
For this reason, the most idiomatic and thus recommended way to retrieve a \clsind{UIManager} instance is the method \mthind{Project class}{uiManager}.
