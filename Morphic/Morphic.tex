...
While we should avoid such situations in general, there is an accessor message, \ct{#currentEvent}, that allows us to retrieve the currently handled event again from every method in every object.
To do so, we can simply write \ct{self currentEvent} in any method.
Analogously, there are \ct{#currentHand} and \ct{#currentWorld} to retrieve the current \clsind{HandMorph} (which represents your cursor) or \clsind{PasteUpMorph} (which contains all other morphs displayed at your screen).

In some cases, one also may want to directly control the user interaction without following the usual event handling control flow.
As an example, we want to track the mouse cursor and find out which point the user clicks next.
Implementing this via the usual handle protocol would be complicated because we want to handle clicks that are not made to a morph controlled by ourselves.%
\footnote{Actually, there is an advanced mechanism called \emph{event-bubbling and -capturing filters} that would be applicable to this problem.
	However, this approach requires a bit more code and is not optimally suited for our simple use case.}
...
However, keep in mind that all these approaches use global state which can lead to problems in particular situations (just imagine a \sq image that deals with multiple cursors\footnote{This is actually the case in some multi-touch implementations for \sq{}.}, which cursor's position are we retrieving now?).
...
SBEScreenshotRecorder writeTo: 'figures/popup.png' building: [:helper |
	...
	menu := PopUpMenu
		...
		lines: #(2 4).
	helper
		runModal: [menu startUpWithCaption: 'Choose a shape']
		then: [helper closeBalloons].
]
\end{ExecuteSmalltalkScript}
