...
%%{SBESqueakPicture writeTo: '../figures/blackHandle.png' frame: #morphs using: [:helper | | workspace |
%%  (HaloMorph new createHandleAt: 0 @ 0 color: Color black iconName: 'Halo-Grab') openInWorld.
%%]}
%%{SBESqueakPicture writeTo: '../figures/moveHandle.png' frame: #morphs using: [:helper | | workspace |
%%  (HaloMorph new createHandleAt: 0 @ 0 color: Color brown iconName: 'Halo-Drag') openInWorld.
%%]}

\indmain{Morphic} is the name of one of \sq's graphical interfaces.
...
%%{SBESqueakPicture writeTo: './figures/detachingMenu.png' frame: [:morphsRect | (morphsRect withBottom: morphsRect bottom - 160) withTop: morphsRect top + 250] using: [:helper | | menuItem haloMorph halo |
%%	helper click: helper world.
%%	menuItem := helper menuItemMorphLabeled: 'new morph...' in: helper world.
%%	helper hand releaseMouseFocus.
%%	helper mouseEnter: menuItem.
%%	menuItem addHalo.
%%	haloMorph := helper morphWithBalloonText: 'Pick up' in: helper world.
%%	halo := haloMorph owner.
%%	helper mousePress: haloMorph at: haloMorph center buttons: #(left).
%%	helper mouseMoveTo: haloMorph center - (10@0) buttons: #() in: halo.
%%	
%%	"The following is a hack as the event simulation and the HaloMorph do not work
%%	nicely together."
%%	halo abandon.
%%	haloMorph := (HaloMorph new createHandleAt: 0 @ 0 color: Color black iconName: 'Halo-Grab') 
%%	center: helper hand center;
%%	openInWorld;
%%	yourself.
%%	haloMorph position: haloMorph position + ((menuItem width / 2)@0).
%%	helper spawnHand.
%%]}
\begin{figure}[ht]
	\ifluluelse
...
The second line opens an ``explorer'' that shows you attributes of this Morph, such as its bounds on the screen.  Clicking on one of the yellow exclamation marks sends a message to the Morph, which responds appropriately.
...
You can place a morph inside another by sending the message \mthind{Morph}{addMorph:} to the container morph. Thereby, you make the added morph a ``submorph''.
...
There are many  methods available to position a morph; browse the \protind{geometry} method category of class \ct{Morph} to see for yourself.
For example, to center the star inside joe, execute \ct{star} \mthind{Morph}{center:} \ct{joe center}.

%%{SBESqueakPicture writeTo: './figures/joeStar.png' frame: #morphs using: [:helper |
%%	| joe bill star |
%%	joe := Morph new color: Color blue.
%%	joe openInWorld.
%%  bill := Morph new color: Color red.
%%  bill openInWorld.
%%	star := StarMorph new color: Color yellow.
%%  {joe . bill . star} do: [:m | m extent: m extent * 2].
%%  joe addMorph: star.
%%  star center: joe center.
%%  joe position: bill center.
%%  joe comeToFront.
%%]}
\begin{figure}[ht]
...
Drag an ellipse and a star from the list.  Place the star over the ellipse, and yellow-click the star to bring up a menu\footnote{You can also blue-click the star to bring up the morphic halo, and then click the red menu handle.}.   Select \menu{embed into \go Ellipse}.  Now the star and the ellipse move together.}
...
To remove a sub-morph, do \ct{joe} \mthind{Morph}{removeMorph:} \ct{star} or \ct{star} \mthind{Morph}{abandon}.  This can also be done by direct manipulation:
...
%%{SBESqueakPicture writeTo: './figures/NewCross.png' frame: #morphs using: [:helper |
%%  | cross |
%%	cross := SBECrossMorph new openInWorld.
%%  cross position: 30@30.
%%  cross extent: 250@250.
%%  cross addHalo.
%%]}
\begin{figure}[hbt]
	\ifluluelse
...
The solution is to factor out these calculations into two new methods, which we put in the \ct{private} method category:
...
\begin{method}{Declare that \ct{CrossMorph} will react to all keyboard events.}
CrossMorph>>>handlesKeyboard: anEvent
	^ true
\end{method}
...
We have written these methods so that you can move the morph using the arrow keys. When the mouse is over the morph, the morph will be asked via the \ct{handlesKeyboard:} method whether it wants to handle the incoming keyboard events.
Note that when the mouse is no longer over the morph, the \mthind{Morph}{keyStroke:} message is not sent, so the morph stops responding to keyboard commands.
To discover the key values, you can open a Transcript window and add \glbind{Transcript} \ct{show: anEvent keyCharacter} to \mthref{handleKeystroke}.
The \ct{anEvent} argument of \ct{keyStroke:} is an instance of \clsind{KeyboardEvent}, another subclass of \clsind{MorphicEvent}. Browse this class to learn more about keyboard events.

\subsection{Morphic animations}

Morphic provides a simple animation system with two main methods: \mthind{Morphic}{step} is sent to a morph at regular intervals of time, while \mthind{Morphic}{stepTime} specifies the time in milliseconds between \ct{step}s.\footnote{\ct{stepTime} is actually the \emph{minimum} time between \ct{step}s. If you ask for a \ct{stepTime} of 1\,ms, and you generate a high system load, \sq might not step your morph that often.}
In addition, 
...
\begin{code}{}
	keyCharacter = $+ ifTrue: [self startStepping].
	keyCharacter = $- ifTrue: [self stopStepping].
\end{code}
...
To prompt the user for input, the \clsind{UIManager} class provides a few ready-to-use interactions. Note, that there is only one UIManager instance active at the time which you can get by sending the message \ct{default} to the class \clsind{UIManager}.
For instance, the \mthind{UIManager}{request:initialAnswer:} method returns the string entered by the user (\figref{dialogName}).


%%{SBESqueakPicture writeTo: './figures/dialog.png' frame: #morphs using: [:helper |
%%	helper
%%    runModal: [UIManager default request: 'What'' your name?' initialAnswer: 'no name']
%%    then: [
%%      helper closeBalloons]
%%]}
\begin{figure}[htb]
	\ifluluelse
		{\centerline{\includegraphics[width=0.55\textwidth]{dialog}}}
		{\centerline{\includegraphics[width=0.55\textwidth]{dialog}}}
	\caption{Dialog displayed by \ct{UIManager default request: 'What''s your name?' initialAnswer: 'no name'}.
...
BorderedMorph subclass: #ReceiverMorph
	instanceVariableNames: ''
	classVariableNames: ''
	poolDictionaries: ''
	category: 'SBE-Morphic'
...
ReceiverMorph>>>initialize
	...
	self bounds: (0@0 extent: 200@200).
\end{method}
...
The dropped morph is also given the opportunity to check and see if it likes the morph onto which it is being dropped; it is sent the message \ct{wantsToBeDroppedInto:}.  The default implementation of this method (in class \ct{Morph}) always answers \ct{true}.
...
%%{SBESqueakPicture writeTo: './figures/die.png' frame: #morphs using: [:helper |
%%  | die |
%%	die := SBEDieMorph new openInWorld.
%%	die dieValue: 5.
%%  die position: 40@40.
%%  die extent: 150@150.
%%  die addHalo.
%%]}
\begin{figure}[ht]
	\centerline{\includegraphics[scale=0.35]{die}}
...
\begin{itemize}
  ...
  \item You can compose morphs by making them submorphs through embedding one onto another, either by drag and drop or by sending the message \ct{addMorph:}.
  ...
  \item The \ct{UIManager} provides several ready-made interactions with users.
\end{itemize}
