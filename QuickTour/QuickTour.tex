...
%%  workspace := ToolBuilder open: Workspace new.
%%  helper type: 'Time now ' into: workspace.
%%  helper click: workspace at: workspace center buttons: #(middle).
%%  "screenshot does currently not include the workspace's halo, please fix"
%%]}
...
%%  "could not find a message to open just the rotate icon"
%%]}
...
%%  (helper preferenceButtonMorphLabeled: 'swapMouseButtons' in: helper world) highlightOn.
%%]}
...
%%  "open world menu, click 'save as...', type 'SBE' into text field"
%%]}
...
% TODO try selecting Sokoban package and first figure out how to get SM to actually search
%%{SBESqueakPicture writeTo: './figures/SqueakMap.png' frame: #morphs using: [:helper |
%%  SMLoaderPlus open.
%%  helper type: 'Sokoban', String cr into: (helper morphWithBalloonText: 'Search packages...' in: helper world)
%%]}
\begin{figure}[htb]
\ifluluelse
...
%%  Browser fullOnClass: Object selector: #printString.
%%  "SBELabelRectangle new text: 'Classes'; center:" 
%%]}
\begin{figure}[htb]
\ifluluelse
...
%%{SBESqueakPicture writeTo: './figures/BooleanHierarchyBrowser.png' frame: [:morphsRect | morphsRect] using: [:helper | | browser browserMorph |
...
%%    then: [helper dialogWindow position: 60 asPoint].
%%  
%%  "This should type 'time' into the dialog window's search panel: (but currently doesn't)
%%  helper dialogWindow messageMorph contents: 'time'."
%%  
%%  helper closeBalloons.
%%]}
...
%%{SBESqueakPicture writeTo: './figures/methodFinder-now.png' frame: #morphs using: [:helper | | browser |
%%  browser := ToolBuilder open: SelectorBrowser new.
%%  "now type 'now' into the window's 4th submorph and press enter.
%%  then click on 'now' in the result list (window's 3rd submorph)"
%%]}
...
%%  "the message template should be highlighted. this can be achieved by clicking on the message category name, which is to be found in the browser's 3rd submorph"
%%]}
...
%%{SBESqueakPicture writeTo: './figures/testShoutConfirm.png' frame: #morphs using: [:helper |
%%  Browser fullOnClass: StringTest category: #'tests - converting'
%%  "type 'testShout
%%	self assert: ('Don''t panic' shout = 'DON''T PANICBANG')'    into the browser's 3rd submorph.
%%   now press cmd-s to save the code.
%%   "
%%]}
...
%%  ToolBuilder open: TestRunner new.
%%  "select 'CollectionsTests-Text' in test runner's 6th submorph,
%%  then click 'Run Selected' button"
%%]}
...
%%    [StringTest new testShout] forkAt: 41.
%%    Processor yield.
%%    helper world doOneCycleNow.
%%    helper takeScreenshot]
%%]}

SBESqueakPicture writeTo: '...' frame: #morphs using: [:helper | | sem |
  helper restoreMethods: {String>>#shout} after: [
    sem := Semaphore new.
    [Error signal] forkAt: 41.
    Processor yield.
    Project current addDeferredUIMessage: [sem signal].
    sem wait.
    helper takeScreenshot]
]}
