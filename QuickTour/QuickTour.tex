...
SBEScreenshotRecorder writeTo: './figures/ClassBrowser2.png' frame: #morphs building: [:helper |
	...
	Dictionary new
		...
		keysAndValuesDo: [:item :description | | list |
			list := helper listMorphIncludingMatch: item in: window.
	...
	(SBEBalloonMorph string: 'method comment' for: codePane)
		popUpTaillessAt: (codePane bounds pointAtFraction: 0.82 @ 0.22).
	(SBEBalloonMorph
		string: ('code of method\"{1}"' withCRs format: {selector})
		for: codePane)
			popUpTaillessAt: (codePane bounds pointAtFraction: 1 / 2 @ (2 / 3)).
]
\end{ExecuteSmalltalkScript}
\begin{figure}[htb]
\ifluluelse
