...
In \figref{download}, we see that we have grabbed the \textit{{\sq}6.0} VM, image, and changes files.
\end{enumerate}
...
Later, you can change the scaling in the menu bar at the top under \menu{Extras~\go{} Scale Factor}.
While you are there, you can also change the looks of the environment via \menu{Extras~\go{} Themes \& Colors}.
...
\end{ExecuteSmalltalkScript}
\begin{figure}[htb]
\centerline {\includegraphics[width=0.9\textwidth]{Doit}}
...
\index{keyboard shortcuts!print it}
...
\seeindex{accept it}{keyboard shortcuts, accept}
...
\begin{figure}[htb]
\ifluluelse
	{\centerline {\includegraphics[width=0.9\textwidth]{testRunnerOnStringTest}}}
...
The window that opens with the error message is the \st debugger (see \figref{quicktour-predebugger}).
