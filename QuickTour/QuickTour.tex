...
Your download will be an archive file that contents vary from platform to platform. However, there are two important components common to all versions (see \figref{download}).
...
\begin{enumerate}
  ...
  \item The \emphind{virtual machine} (VM) is the only part of the system that is different for each operating system and processor.  Pre-compiled virtual machines are available for all the major computing environments.  In \figref{download} we see the VM for the Windows is called \textit{\sq.exe}.
...
In \figref{download}, we see that we have grabbed the \textit{{\sq}5.2-18231} image and changes files.
...
%%		extent: 800 @ 450.
%%	balloon := Form fromBinaryStream: (helper resourceDirectory
%%		/ 'branding' fileNamed: 'balloon.png').
%%	ReleaseBuilder openWelcomeWorkspacesWith: balloon.
%%	helper world firstSubmorph
%%		height: helper world height * 0.75;
%%		center: helper world center.
%%	PreferenceWizardMorph open.
%%	self flag: #ct "After Objectland has been merged into Trunk, simplify this call and don't ask PreferenceWizardMorph".
%%]}
\begin{figure}[htb]
...
\begin{figure}[tbh]
	...
	\vspace{0.1in}
		...
		%\ct{TODO: Display this figure between the other figures, technically overlap them. I did not "figure" out how to do without installing extra packages.}
		\includegraphics[width=0.4\linewidth]{morphicHaloOnWorkspace}}% these braces needed (else no whitespace at end of line)
	\end{center}		
...
%%  (HaloMorph new createHandleAt: 0 @ 0 color: Color blue iconName: 'Halo-Rot') openInWorld.
%%]}
...
%%	browser := helper world firstSubmorph.
%%	browser extent: 600 @ 275.
%%	searchBox := helper morphWithBalloonText: 'Search preference.*' asRegex in: browser.
...
%%	appsItem := helper menuItemMorphLabeled: 'Apps' in: dockingBar.
...
%%	(helper windowLabeled: 'SmallInteger.+' asRegex)
...
%%					popUp].
%%	codePane := helper textMorphIn: window withTextMatching: 'printString[\s\S]*' asRegex.
...
%%	3 "scroll the selection nicely into view"
...
%%			helper
%%				type: 'testShout\\	self assert: (''Don''''t panic'' shout = ''DON''''T PANIC!'')' withCRs into: textField;
...
%%		runAll] ensure: [StringTest removeSelector: #testShout]
%%]}
