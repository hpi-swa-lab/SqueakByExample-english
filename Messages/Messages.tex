...
You should note that not all message sends have arguments.
Unary messages like \ct{open} do not have arguments.
Single keyword and binary messages like \ct{go: 100} and \ct{+ 20} each have one argument. 
There are also simple messages and composed ones.
...
\begin{itemize}
\item \emph{Unary messages} are messages that are sent to an object without any other information.
...
Hence \ct{r:g:b:} is a method with three arguments, \ct{playFileNamed:} and \ct{at:} are methods with one argument, and \ct{at:put:} is a method with two arguments.
...
\important{In \ind{Java} or \ind{C++}, the \st method invocation \ct{Color r: 1 g: 0 b: 0} would be written \ct{Color.rgb(1,0,0)}.}
...
\caption{Unary messages are sent first so \ct{Color yellow} is sent.
	This returns a color object which is passed as an argument of the message \ct{aPen color:}.\label{fig:uKeyUn}}
\end{figure}
...
\footnote{In fact, we could also have written the equivalent but simpler expression: \ct{Boolean methodDict select: #isAbstract thenCollect: #selector}}\damien{I've added this footnote, just remove it if you don't like it :-)%
}.
...
This returns a color object which is passed as an argument of the message \ct{aPen color: aColor} (2) as shown in \egref{decColor}.
...
\paragraph{Example.}
In the message \ct{aPen go: 100 + 20}, there is a \emph{binary} message \ct{+ 20} and a \emph{keyword} message \ct{go:}.
...
Then the message \ct{aPen go: 120} is sent with \ct{120} as argument (2).
...
\paragraph{Example.}
As an exercise, we let you decompose the evaluation of the message \ct{Pen new go: 100 + 20} which is composed of one unary, one keyword, and one binary message (see \figref{unKeyBin}).
...
They are sent from the left to the right. Note that you already saw this behavior in \egref{decExtent} where the two point creation messages (\ct{@}) were sent first.
...
Here, the leftmost message \ct{+} is sent first (1) and then the \ct{*} is sent to the result as shown in \egref{binaryMessages1}.  
...
The characters \ct{[}, \ct{]}, \ct{(} and \ct{)} delimit distinct areas.
Within such an area, a keyword message is the longest sequence of words terminated  by \ct{:} that is not cut by the characters \ct{.} or \ct{;}. 
...
\important{
	The characters \lct{[}, \lct{]}, \lct{(} and \lct{)} delimit distinct areas.
	Within such an area, a keyword message is the longest sequence of words terminated by \lct{:} that is not cut by the characters \lct{.} or \lct{;}.
...
Hence the conditional branches of \ct{ifTrue:} or \ct{ifTrue:ifFalse:} require blocks.
